...
if lsp_util.is_windows() then
    ...
    script_to_use=
[[ (for /f "tokens=1,* delims=:" %a in ('curl -s https://api.github.com/repos/sumneko/vscode-lua/releases/latest ^| findstr "browser_" ') do curl -L -o sumneko-lua.vsix %b) && ECHO cleaning && del /F /Q sumneko-lua & mkdir sumneko-lua & tar -xf sumneko-lua.vsix - -C sumneko-lua && del /F /Q sumneko-lua.vsix && echo sumneko-lua\extension\server\bin\Windows\lua-language-server -E -e LANG=en sumneko-lua\extension\server\main.lua >> sumneko-lua-language-server.cmd
]]
end
